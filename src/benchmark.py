...
class SimpleTargetEncoder(TransformerMixin, BaseEstimator):
    """
    Simple target encoder that maps categorical values to the mean target value.
    - For classification uses class label mean (works with binary/multi by mapping to numeric)
    - For regression uses numeric target mean
    This is intentionally simple and robust for demo/benchmarking purposes.
    """
...
# ----------------------
# Evaluate a pipeline for a given column
# ----------------------
...
def benchmark_column(
    ...
    """
    Benchmark preprocessing strategies for a specific column in df.
    ...
    column : str
        Column to benchmark preprocessing for
    target : str
        Name of the target column in df
    problem_type : str
        'classification' or 'regression'
    strategies : list of dict (optional)
        If None, use default sets defined above
    cv : int
        Number of CV folds
    scoring : str
        sklearn scoring name. Defaults to 'f1_macro' for classification, 'r2' for regression.
    n_jobs : int
        Number of parallel workers for cross_val_score
    bo : bool
        Whether to run Bayesian optimisation to refine strategy choice
    bo_iterations : int
        Number of BO iterations (if bo=True)
    ...
    dict
        {
            ...
            "best_score": float,
            "all_results": dict of strategy_name->score,
            "bo_trace": optional (return of optimizer)
        }
    """
    if scoring is None:
...
# ----------------------
# Quick CLI test (minimal example)
# ----------------------
